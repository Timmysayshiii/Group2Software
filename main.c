...
int main(void){
  ...
  pointer = nmeasentence;
	
	printf("\n\nNmea sentence to decode = ");
	for(int i=0; i<77; i++){
		printf("%c", nmeasentence[i]);
}
...
void storeLatitude(){
  for(int i=0; i<14; i++){
	latitudefromstream[x] = nmeasentence[x+17];
	x = x+1;
	}
...
void storeLongitude(){
  for(int i=0; i<15; i++){
	longitudefromstream[z] = nmeasentence[z+32];
	z = z+1;
	}
...
void storeAltitude(){
  for(int i=0; i<7; i++){
	altitudefromstream[t] = nmeasentence[t+58];
	t = t+1;
	}
